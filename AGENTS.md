...
|                              v                                                                          |
|                 [Action: SafeDelete]                                                                    |
|                 DeleteEngine (Soft Trash & Permanent)                                                   |
+---------------------------------------------------------------------------------------------------------+
```
...
- **Google Material Design 3 Spec & Plans:** [`docs/superpowers/specs/2026-09-18-google-material-design-system.md`](file:///home/shubhamshah207/clairvoy/docs/superpowers/specs/2026-09-18-google-material-design-system.md) & [`docs/superpowers/plans/2026-09-18-google-material-ui-and-delete.md`](file:///home/shubhamshah207/clairvoy/docs/superpowers/plans/2026-09-18-google-material-ui-and-delete.md)
  - Details the Google Photos product interface, morphing Top Selection Bar (zero floating bottom dock), left sidebar category navigation with filter-clearing crosses (Photos, Videos, Screenshots, Documents, Other Files, All), left-bottom persistent storage indicator, Small/Medium/Large thumbnail size controls, video keyframe thumbnails (`/api/thumbnail`) and media streaming (`/api/media`), full-screen lightbox viewer with side-by-side diff, and Safe Trash & Permanent Deletion engine.
- **Premier UX Design System Spec & Plans:** [`docs/superpowers/specs/2026-09-18-premier-ux-design-system.md`](file:///home/shubhamshah207/clairvoy/docs/superpowers/specs/2026-09-18-premier-ux-design-system.md) & [`docs/superpowers/plans/2026-09-18-premier-ux-redesign.md`](file:///home/shubhamshah207/clairvoy/docs/superpowers/plans/2026-09-18-premier-ux-redesign.md)
  - Details the dark-mode UI foundation, Smart Clean Recommendation Hero Banner (1-click trash/quarantine & auto-rule presets), cohesive M3 duplicate cluster cards with primary filenames and path hints, Google Emerald keeper highlighting with 1-click candidate promotion, rich keyboard shortcuts (Space, K, X, Esc, ?), and interactive toast notification system.
  - Zero auto-selection on refresh: uses explicit opt-in `selectedPaths` so fresh page loads start at 0 selected, keeping the default search/nav header active until items are chosen.
  - Google Drive Left Rail Architecture: `+ New Scan` primary action and `Scan Run` session switcher (`#runsDropdown`) are housed at the top of the left sidebar rail, leaving the top-right header clean, decluttered, and focused on utilities.
  - Google Workspace Storage Scan Modal (`#newScanModal`): Replaced text drawer with a dedicated scan modal featuring native OS folder picker (`POST /api/system/pick-folder`), in-app Google Drive directory browser (`GET /api/system/browse-directories` & `#driveBrowserModal`), quick shortcut chips, custom path input, target chips, and similarity slider.
  - Dual Cleaning Recommendation Panels: Distinct split tracks between 100% byte-exact duplicates (`#heroExactGb`, direct 1-click clean to Trash/Quarantine without manual inspection) and similarity/near-duplicates (`#heroSimilarGb`, side-by-side diff comparison review required).
  - Match Confidence Filters: Segmented header toggle (`All`, `⚡ Exact`, `🔍 Similar`) and Left Rail sidebar category filters (`⚡ 100% Exact`, `🔍 Similar (Review)`).
